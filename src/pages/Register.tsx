    ...
    <Card className="border-border shadow-2xl">
      <CardContent className="p-8 text-center">
        ...
        <p className="text-muted-foreground">
          Thank you for registering for <strong>{eventName}</strong>. You'll
...
const EventInfo = ({
  ...
  return (
    <div className={`pt-6 md:pt-0 ${className}`}>
      ...
      <h1 className="text-2xl sm:text-4xl md:text-7xl font-display font-bold">
        {event.name}
      </h1>
      {description && (
...
const RegistrationForm = ({
  ...
  <form onSubmit={onSubmit} className={`space-y-4 ${className}`}>
    ...
    <div className="flex items-start gap-2 pt-2">
      ...
      >
        I agree to receive communications about this event and consent to the
        processing of my data in accordance with the Privacy Policy.
      </Label>
    </div>
    <Button
      type="submit"
...
const PoweredBy = () => (
  <p className="text-center text-xs text-muted-foreground mt-6">
    Powered by <span className="font-semibold">nextevents</span>
  </p>
);
...
const Register = () => {
  ...
  const { data: formFields, isLoading: fieldsLoading } = useFormFields(
    event?.id,
  );
  ...
  const wrapDark = (content: React.ReactNode) => (
    <div className={isDark ? "dark" : ""}>{content}</div>
  );

  if (submitted) {
    return wrapDark(
      <div
        className="min-h-screen flex items-center justify-center px-4 py-12 bg-background text-foreground"
        style={{
          background: isDark
            ? undefined
        ...
        <div className="md:w-1/2 bg-muted flex items-start md:items-center justify-center p-0 md:p-6 min-h-[250px] md:min-h-screen">
          {flyerUrl ? (
            <img
              src={flyerUrl}
              alt={event.name}
              className="w-full md:max-w-full md:max-h-full object-contain md:rounded-lg"
            />
          ) : (
  ...
  if (template === "stacked") {
    return wrapDark(
      ...
      >
        {flyerUrl ? (
          <div
            className="w-full bg-muted flex items-start justify-center"
      ...
      <div className="min-h-screen bg-background text-foreground">
        ...
        >
          ...
          <div className="relative text-center text-white px-4 py-16 z-10">
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-display font-bold mb-4 truncate max-w-3xl mx-auto">
              {event.name}
            </h1>
            {event.description && (
              <p className="text-white/80 max-w-xl mx-auto text-lg leading-relaxed mt-3">
                {event.description}
              </p>
            )}
